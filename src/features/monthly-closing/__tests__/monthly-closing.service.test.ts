...
describe("monthly-closing.service — benefitEvolution", () => {
  ...
  it("estende a janela para o crédito anterior e inclui movimentos após o periodEnd", async () => {
    ...
    expect(closing.benefitEvolution.lastCreditDate).toBe("2027-05-01")
    expect(closing.benefitEvolution.creditDates).toEqual(["2027-05-01"])
    expect(closing.benefitRechargeAlert.late).toBe(false)
    expect(closing.benefitRechargeAlert.referenceMonth).toBe("2027-04")
