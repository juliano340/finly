...
import { payFixedCostOccurrenceWithCard } from "@/features/monthly-closing/monthly-closing.service"
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!fixedCost.paidInsideCard) {
    return NextResponse.json({ error: "Custo fixo não está dentro do cartão" }, { status: 400 })
  }
  ...
  const paid = await payFixedCostOccurrenceWithCard(occurrence.id, userId, prisma)
  if (!paid) {
    return NextResponse.json({ error: "Erro ao pagar com cartão" }, { status: 400 })
  }
